...
class PushNotificationAuth:
    def _calculate_request_body_sha256(self, data: dict[str, Any]) -> str:
        ...
        This logic needs to be same for both the agent who signs the payload
...
class PushNotificationSenderAuth(PushNotificationAuth):
    ...
    def _generate_jwt(self, data: dict[str, Any]) -> str:
        """JWT is generated by signing both the request payload SHA digest
        and time of token generation.

        Payload is signed with private key and it ensures the integrity of
        payload for client. Including iat prevents from replay attack.
        """
        iat = int(time.time())
    ...
    async def send_push_notification(self, url: str, data: dict[str, Any]) -> None:
...
class PushNotificationReceiverAuth(PushNotificationAuth):
    def __init__(self) -> None:
        self.public_keys_jwks: List[Dict[str, Any]] = []
